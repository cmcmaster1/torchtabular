...
```{r}
is_heads <- intersample_attention_heads(fitted, valid_dset, n = 2000, cut = 0.98)
```
...
set.seed(132)
mapped <- umap(is_heads, pca = NULL, n_threads = 4, n_epochs = 500, spread = 3, min_dist = 0.0, n_neighbors = 10)
...
```{r}
scanned <- dbscan(umap_comp, eps = 3.0, MinPts = 20)
plotting_data %>% 
