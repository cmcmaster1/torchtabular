...
We will use a callback to reduce the learning rate by 90% after 10 epochs.

```{r}
lambda <- function(epoch){
  if (epoch > 10) 0.1 else 1
}

lr_decay <- luz::luz_callback_lr_scheduler(torch::lr_lambda, lr_lambda = lambda)
```
...
```{r}
is_heads <- intersample_attention_heads(fitted, valid_dset, n = 2000, cut = 0.00)
```
